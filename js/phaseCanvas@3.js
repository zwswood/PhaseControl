...
(function($, window, document) {
	var Phase = function(options) {
		...
		this.arrayDraw = []; // 将绘制的可用信号组对象数组
		this.greenMap = new Map(); // 绿灯信号组
		this.allSignalMap = new Map(); // 所有的信号组
		// 定义所有信号组显示名称
	...
	Phase.prototype = {
		...
		drawSignalLight: function() {
			...
			if (_self.options.greenSignal) { // 绘制绿灯信号组			
				var useArray = _self.options.usableSignal.split(',');
				_self.filterData(useArray);
				var greenGroup = _self.options.greenSignal.split(',');
				_self.filterData(greenGroup); // 传入参数中同时存在11P,12P,1P时,过滤掉11P,12P...	
				// 从绿灯信号组中移除可用信号组中不存在的信号组
				for (let i = greenGroup.length - 1; i >= 0; i--) {
					let value = greenGroup[i];
					if (useArray.indexOf(value) < 0) {
						greenGroup.splice(i, 1)
					}
				}
				for (var key in greenGroup) {
					// 将绿灯信号组加入greenMap
					_self.greenMap.set(greenGroup[key], _self.allSignalMap.get(greenGroup[key]) + ',0');
		...
		event: function() {
			...
			_self.oCanvas.addEventListener('dblclick', function(e) {
				...
				for (var i = 0; i < _self.arrayDraw.length; i++) {
					if (_self.arrayDraw[i].disp.indexOf('P') > 0) {
						usablePerson.push(_self.arrayDraw[i].disp);
					} else {
						usableCar.push(_self.arrayDraw[i].disp);
					}
				}
				...
				for (var i = 0; i < _self.arrayDraw.length; i++) {
					...
					if (p && _self.canContext.isPointInPath(p.x, p.y)) { // 判断鼠标位置是否属于热点区域
						...
						if (_self.arrayDraw[i].iy < 100) { // 亮灯
							...
							if (clashSignal) {
								var tempClash = clashSignal.split(',');
								for (let j = 0; j < tempClash.length; j++) {
									if (tempClash[j].indexOf('P') > 0) {
										clashPerson.push(tempClash[j]);
									} else {
										clashCar.push(tempClash[j]);
									}
								}
							}
							// 检测机动车灯冲突
							let isClash = false; //是否冲突
							let clashDisp = '';
							for (let m = 0; m < clashCar.length; m++) { // 循环数组
								_self.arrayDraw.map(t => {
									if (t.disp == clashCar[m]) {
							...
							if (!isClash) { // 不冲突
								...
								// 将点亮了的灯组加入_self.greenMap					
								_self.greenMap.set(_self.arrayDraw[i].disp, _self.allSignalMap.get(_self.arrayDraw[i].disp) + ',0');

								// 从可用行人信号组中移除冲突的行人信号组
								/*for (let i = usablePerson.length - 1; i >= 0; i--) {
									let value = usablePerson[i];
									if (clashPerson.indexOf(value) > -1) {
										usablePerson.splice(i, 1)
									}
								} */

								// 点亮不冲突的所有可用行人灯
								/* for (let a = 0; a < usablePerson.length; a++) {
									_self.arrayDraw.map(t => {
										if (t.disp == usablePerson[a]) {
											if (t.iy > 100) {
												t.iy = t.iy - 600;
												_self.canContext.clearRect(t.x, t.y, t.w, t.h);
											}
											_self.canContext.drawImage(_self.drawImg, t.ix, t.iy + 600, t.w, t.h, t.x,
												t.y, t.w, t.h);
											t.iy = t.iy + 600;
											_self.greenMap.set(t.disp, _self.allSignalMap.get(t.disp) + ',0');
										}
									});
								} */
								// 灭掉冲突的行人灯
								for (let b = 0; b < clashPerson.length; b++) {
									_self.arrayDraw.map(t => {
										if (t.disp == clashPerson[b]) {
											if (t.iy > 100) {
												t.iy = t.iy - 600;
												_self.canContext.clearRect(t.x, t.y, t.w, t.h);
											}
											_self.canContext.drawImage(_self.drawImg, t.ix, t.iy, t.w, t.h, t.x,
												t.y, t.w, t.h);
											_self.greenMap.delete(t.disp);
										}
									});
								}
